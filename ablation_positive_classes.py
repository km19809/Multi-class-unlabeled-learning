...
# Date: 20/04/2022
#
# In this script we show the different scores when varying the number of positive classes
...
if __name__ == '__main__':
    ...
    # start execution
    for num_positive_classes in [2, 4, 6, 8, 9]: # different number of positive classes

        print("\n\n-------------------------- NUM POSITIVE CLASSES:", num_positive_classes)

        # prefix for the folder log path
        prefix_path = datetime.datetime.now().strftime("%m_%d_%H") + "_positive_class" + str(num_positive_classes) + "_"
        ...
        for dataset_name in datasets:
            for name in classifiers:
                ...
                # get model
                if name == "linearSVM":
                    model = LinearSVM(name, dataset_name, perc_ds, perc_labeled, data_preparation, n_runs, prefix_path, num_neg_classes, validation_hyp,generate_dataset, num_positive_classes)
                elif name == "rbfSVM":
                    model = RbfSVM(name, dataset_name, perc_ds, perc_labeled, data_preparation, n_runs, prefix_path, num_neg_classes, validation_hyp,generate_dataset, num_positive_classes)
                elif name == "area":
                    model = AREA(name, dataset_name, perc_ds, perc_labeled, data_preparation, n_runs, prefix_path, num_neg_classes, validation_hyp,generate_dataset, num_positive_classes)
                elif name == "urea":
                    model = UREA(name, dataset_name, perc_ds, perc_labeled, data_preparation, n_runs, prefix_path, num_neg_classes, validation_hyp,generate_dataset, num_positive_classes)
                elif name == "protoMPUL":
                    model = ProtoMPUL(name, dataset_name, perc_ds, perc_labeled, data_preparation, n_runs, prefix_path, num_neg_classes, validation_hyp, generate_dataset, num_positive_classes)
