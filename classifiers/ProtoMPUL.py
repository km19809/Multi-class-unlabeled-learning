...
# Implementation of the ProtoMPUL method (Prototype based Multi-Positive and Unlabelled Learning)
# For more info: https://ieeexplore.ieee.org/document/9773176
class ProtoMPUL(bc.BaseClassifier):
    ...
    def get_sup_loss(beta_same=1., beta_diff=1.):
        ...
        def my_sup_loss(y_true, y_pred):
            ...
            # just take the upper part of the square matrix (the distance matrix is symmetric)
    ...
    @staticmethod
    def get_stacked_model(input_dim, output_dim, first_pair, last_pair):
    ...
    def get_grid_hyperparameters(self):
        ...
        # Those two margins are always the same and they are equal to Beta_sup
    ...
    def train_model(self, model, ds_labeled, y_labeled, ds_unlabeled, y_unlabeled, x_test, y_test, current_hyp):
        ...
        def run_stage2(model_unlabeled, model_labeled, ds_labeled,
        ...
        def run_stage3(model_unlabeled, model_labeled, ds_labeled_original, y_labeled_original,
        ...
        # first stage
        # setting initial weights for autoencoder if the stacked pretraining is enabled
        if self.do_stacked_pretraining:
        ...
        # second stage
        history_pre = run_stage2(model_unlabeled, model_labeled, ds_labeled, y_labeled, ds_unlabeled, epochs_pretraining)
        ...
        # third stage
        history_clu, epochs_clu, data_plot = run_stage3(model_unlabeled, model_labeled,
    ...
    def run_preparation(self, ds_labeled, y_labeled, ds_unlabeled):
        ...
        # setting the net dimensions
        self.data_for_run['dims'] = [ds_labeled[0].shape[0], 500, 500, 2000, 10]
        dims = self.data_for_run['dims']

        # stage 1 is implemented here, no hyper-parameters needed
        if self.do_stacked_pretraining:
            # define stacked models
            ds_all = np.concatenate((ds_labeled, ds_unlabeled), axis=0)
            models_stacked = []

            epochs_stacked = 200  # number of epochs for the greedy layer-wise pretraining
            ...
            self.data_for_run['models_stacked'] = models_stacked  # store trained models

    def get_centroids_from_kmeans(self, model_labeled, x_unlabeled, x_labeled, y):
        '''This method returns centroids for third stage of the protoMPUL algorithm'''

        def get_initial_centroids(X, k, centers=None):
            '''This method returns initial centroids for the kmeans algorithm'''
...
# Custom layer for the clustering component of protoMPUL
