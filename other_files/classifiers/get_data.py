...
def get_data(positive_classes, negative_class, perc_labeled, flatten_data=False,
    ...
    x_val = np.array([x for i, x in enumerate(x_data) if tot_test <= i < (tot_val + tot_test)])
    y_val = np.array([x for i, x in enumerate(y_data) if tot_test <= i < (tot_val + tot_test)])

    x_train = np.array([x for i, x in enumerate(x_data) if i >= tot_test + tot_val])
    y_train = np.array([x for i, x in enumerate(y_data) if i >= tot_test + tot_val])
