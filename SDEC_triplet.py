...
from keras.layers import Dense, Layer, InputSpec, Lambda, Subtract
...
class SDEC(bc.BaseClassifier):
    ...
    def triplet_autoencoder(self, encoder, input_shape):
        """
        ...
        distance1 = Lambda(self.squared_euclidean_distance,
                           output_shape=self.eucl_dist_output_shape)([enc1, enc2])

        distance2 = Lambda(self.squared_euclidean_distance,
                           output_shape=self.eucl_dist_output_shape)([enc1, enc3])

        distance3 = Lambda(self.squared_euclidean_distance,
                           output_shape=self.eucl_dist_output_shape)([enc2, enc3])
        ...
        model = Model(inputs=[input_a, input_b, input_c], outputs=outputs, name="tripletNet")
        return model
    ...
    def set_model_output(self, input_data, encoded, decoded, hyp, include_clustering=False, centroids=None):
        ...
        gamma_supervised = hyp['Gamma_sup']

        # fixed gamma for the clustering loss component
        gamma_kld = 0.1

        loss_labeled = [self.triplet_loss, self.triplet_loss]
        loss_weights_labeled = [gamma_supervised, gamma_supervised]
        ...
        model_labeled = self.triplet_autoencoder(encoder, tuple(input_data.shape[1:]))
    ...
    def extractTriplet(self, ds_labeled, y_labeled):

        # the number of triplets taken is the same number of labeled instances
        nrow = len(ds_labeled)
        ...
        while True:
            ...
            i, j, k = np.sort(np.random.choice(range(len(ds_labeled)), 3, False))

            if y_labeled[i] == y_labeled[j] and y_labeled[i] != y_labeled[k]:
                f_data.append(ds_labeled[i])
                s_data.append(ds_labeled[j])
                t_data.append(ds_labeled[k])

        return np.array(f_data), np.array(s_data), np.array(t_data)
    ...
    def train_model(self, model, ds_labeled, y_labeled, ds_unlabeled, y_unlabeled, x_test, y_test, current_hyp):
        ...
        def run_pretraining(model_unlabeled, model_labeled, ds_labeled,
                            y_labeled_original, ds_unlabeled_original, epochs_pretraining):
            ...
            ds_unlabeled = np.concatenate((ds_unlabeled_original, ds_labeled), axis=0)
            ...
            while epoch < epochs_pretraining:
                ...
                y_labeled_original = y_labeled_original[shuffler_l]

                shuffler_l = np.random.permutation(len(ds_unlabeled))
                ds_unlabeled = ds_unlabeled[shuffler_l]

                f_data, s_data, t_data = self.extractTriplet(ds_labeled, y_labeled_original) # get triplets
                ...
                while not (finish_labeled and finish_unlabeled):
                    ...
                    if not finish_labeled:
                        if (i_lab + 1) * bs_lab >= f_data.shape[0]:

                            t_labeled = [fake_y_labeled[i_lab * bs_lab::],
                                         fake_y_labeled[i_lab * bs_lab::]]
                            b_labeled = [f_data[i_lab * bs_lab::],
                                         s_data[i_lab * bs_lab::],
                                         t_data[i_lab * bs_lab::]]

                            finish_labeled = True
                        else:
                            t_labeled = [fake_y_labeled[i_lab * bs_lab:(i_lab + 1) * bs_lab],
                                         fake_y_labeled[i_lab * bs_lab:(i_lab + 1) * bs_lab]]

                            b_labeled = [f_data[i_lab * bs_lab:(i_lab + 1) * bs_lab],
                                         s_data[i_lab * bs_lab:(i_lab + 1) * bs_lab],
                                         t_data[i_lab * bs_lab:(i_lab + 1) * bs_lab]]
        ...
        def run_clustering(model_unlabeled, model_labeled, ds_labeled_original, y_labeled_original,
            ...
            while batch_n < maxiter and not stop_for_delta:
                ...
                ds_labeled_original = ds_labeled_original[shuffler_l]
                y_labeled_original = y_labeled_original[shuffler_l]

                f_data, s_data, t_data = self.extractTriplet(ds_labeled_original, y_labeled_original)  # get triplets
                fake_y_labeled = np.ones(shape=(len(f_data, )))
                ...
                while not (finish_labeled and finish_unlabeled):
                    ...
                    if batch_n % self.update_interval == 0:
                        ...
                        q = model_unlabeled.predict(ds_unlabeled)[1]
        ...
        centroids = self.get_centroids_from_kmeans(encoder, ds_unlabeled, ds_labeled, y_labeled)
    ...
    def get_centroids_from_kmeans(self, model_labeled, x_unlabeled, x_labeled, y):
        ...
        all_x_encoded = model_labeled.predict(all_ds)  # get points in the embedded space

        # getting initial centroids for positive classes
        centroids = []
        if len(x_labeled) > 0:
            x_labeled_encoded = model_labeled.predict(x_labeled)  # encoded points
