...
def main():
    ...
    model_loaded = False
    name_file_model = 'parameters/test_also_classification'
    ...
    output_classification = Dense(len(classes), activation="softmax", name="output")(encoder.output)

    # si costruisce il modello totale
    model = Model(inputs=encoder.input, outputs=[cl, autoencoder.output, output_classification])

    plot_model(model, to_file='model2.png', show_shapes=True)
    Image(filename='model2.png')

    # run k means for cluster centers todo mettere i cluster iniziali
    #initial_clusters = []
    ...
    run_inter(model, x, y, y_pred_last, index_array, batch_size, maxiter, tol, update_interval, 0.0, 1, 0.1)
...
def run_inter(model, x, y, y_pred_last, index_array, batch_size, maxiter, tol,
              update_interval, kld_weight, mse_weight, cat_weight=0.2):
    ...
    model.compile(loss=['kld', 'mse', 'categorical_crossentropy'], loss_weights=[kld_weight, mse_weight, cat_weight], optimizer=Adam())

    loss = [0, 0, 0, 0]
    index = 0
    for ite in range(int(maxiter)):
        if ite % update_interval == 0:
            q, _, _ = model.predict(x, verbose=0)
        ...
        if (index + 1) * batch_size > x.shape[0]:
            loss = model.train_on_batch(x=x[index * batch_size::],
                                             y=[p[index * batch_size::],
                                                x[index * batch_size::],
                                                y_categorical[index * batch_size::]])
            index = 0
        else:
            loss = model.train_on_batch(x=x[index * batch_size:(index + 1) * batch_size],
                                             y=[p[index * batch_size:(index + 1) * batch_size],
                                                x[index * batch_size:(index + 1) * batch_size],
                                                y_categorical[index * batch_size:(index + 1) * batch_size]])
            index += 1
